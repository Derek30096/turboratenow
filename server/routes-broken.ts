...
export function registerRoutes(app: express.Application) {
  // Serve the beautiful React app for custom domain
  app.use('*', (req, res, next) => {
    const host = req.header('host');
    if (host === 'turboratenow.com' || host === 'www.turboratenow.com' || host?.includes('turboratenow')) {
      console.log(`🚀 DOMAIN ACTIVE: ${host} ${req.url} - Serving React app`);
      // Let the React app handle the domain requests instead of static HTML
      next();
      return;
    }
    next();
...
<body>
  ...
  // Simple request logging without domain override
  app.use((req, res, next) => {
